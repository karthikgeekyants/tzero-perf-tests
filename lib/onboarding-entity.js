...
import {
  ...
  ONBOARDING_SUBMIT_MAX_ATTEMPTS,
  ONBOARDING_SUBMIT_RETRY_DELAY_SECONDS,
...
export function runEntityOnboardingFlow(accountType, userPrefix) {
  ...
  for (let attempt = 1; attempt <= ONBOARDING_SUBMIT_MAX_ATTEMPTS; attempt++) {
    res = http.post(url(`/uap/v1/accounts/${accountId}/submit`), JSON.stringify({}), {
      headers: jsonHeaders(),
      tags: { name: 'SubmitOnboardingAccount' },
    });
    if ((res.status !== 503 && res.status !== 500) || attempt === ONBOARDING_SUBMIT_MAX_ATTEMPTS) break;
    sleep(ONBOARDING_SUBMIT_RETRY_DELAY_SECONDS);
  }
