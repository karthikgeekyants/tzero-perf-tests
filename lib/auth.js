...
import { SESSION_REFRESH_INTERVAL_MS } from '../config/environment.js';
...
// Returns whether the send request itself succeeded -- not a real code.
// The response's `code` field is a status label (e.g. "SENT"), never the
// actual OTP, which only ever reaches the user's real inbox. Nothing in this
// codebase can retrieve or bypass that, so callers exercise this endpoint's
// traffic without gating on a real verification step -- see
// EMAIL_VERIFICATION_CODE in config/environment.js for that placeholder.
export function sendEmailVerificationCode(email) {
  ...
  return check(res, { 'send email verification code ok': (r) => r.status === 200 });
}
