...
// Same idea as pickCapturedSignupPoolUser() above, but for accounts that
// completed full onboarding (not just sign-up) -- captured from a real
// scenarios/02-onboarding-individual.js / 02-onboarding-entity.js run's own
// output (see scripts/build-onboarding-run-pool.js). Unlike
// pickOnboardedPooledUser() above, these carry the real (random) email
// directly rather than a deterministic pattern, since that's what a real
// onboarding run produces -- lets Invest/Trade/Order History reuse whichever
// real onboarded accounts a run actually created and got dev-approved,
// instead of needing the separate deterministic-pattern seed script.
let capturedOnboardingIndividualPool = [];
try {
  capturedOnboardingIndividualPool = JSON.parse(open('../config/onboarding-individual-run-pool.json'));
} catch (e) {
  // Not built yet -- pickCapturedOnboardedPoolUser() surfaces this via a missing totpSecret.
}

export function pickCapturedOnboardedPoolUser() {
  if (capturedOnboardingIndividualPool.length === 0) {
    return { email: '', password: SIGN_UP_PASSWORD, clientId: SIGNUP_CLIENT_ID, totpSecret: undefined };
  }
  const entry = capturedOnboardingIndividualPool[__VU % capturedOnboardingIndividualPool.length];
  return {
    email: entry.email,
    password: entry.password,
    clientId: SIGNUP_CLIENT_ID,
...
let capturedOnboardingEntityPool = [];
try {
  capturedOnboardingEntityPool = JSON.parse(open('../config/onboarding-entity-run-pool.json'));
} catch (e) {
  // Not built yet -- pickCapturedOnboardedEntityPoolUser() surfaces this via a missing totpSecret.
}

export function pickCapturedOnboardedEntityPoolUser() {
  if (capturedOnboardingEntityPool.length === 0) {
    return { email: '', password: SIGN_UP_PASSWORD, clientId: SIGNUP_CLIENT_ID, totpSecret: undefined };
  }
  const entry = capturedOnboardingEntityPool[__VU % capturedOnboardingEntityPool.length];
  return {
    email: entry.email,
    password: entry.password,
    clientId: SIGNUP_CLIENT_ID,
